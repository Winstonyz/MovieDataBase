...
async function getMovie(req,res){
    ...
    if (sfield == "moviename"){
        ...
        connection.query(`
        select movie.title Name,p.name Director, movie.year Release_year, movie.runtime Runtime,null Writer, null Genre
        from movie
        ...
        connection.query(`
        select movie.title Name,null Director, movie.year Release_year, movie.runtime Runtime,p.name Writer, null Genre
        from movie
...
limit 100;
        ...
        connection.query(`
        select m.*,r.avgRating from movie m
        inner join rating r
    ...
    limit 100;
        ...
        connection.query(`
        select m.title Name,null Director, m.year Release_year, m.runtime Runtime,null Writer, g.name Genre
        from movie m
...
async function getPopularMovie(req,res){
    ...
    const genre = req.query.genre ? req.query.genre : "";
        ...
        connection.query(`select m.*,gO.name genre, r.avgRating from rating r
...
async function findKeyword(req, res) {
    const keywordPattern = req.query.keywordPattern ? req.query.keywordPattern : "";

    connection.query(`SELECT m.title, m.year, m.runtime
FROM movie m
...
async function getPost(req, res) {
    ...
    connection.query(`select m.tconst,m.year ReleaseYear,m.runtime Runtime,z.name Genre,x.name Director,y.name Writer,Round(s.avgRating,1) Rating
         from movie m
                  left join (
